...
WORD_PAT_RE = re.compile(r"^[A-Za-z]+$")         # 匹配由字母组成的单词，多词语会排除掉
...
def gap_score(
    ...
    # GAP without MWEs
    # 在计算时排除掉多词表达（Multi-Word Expressions，MWE）。这里用正则表达式 WORD_PAT_RE 过滤掉不符合单词模式的词。
    # 看的是这个指标
    gold_map_normalized = {
        word: weight
...
def compute_gap_nominator(
    ...
    """
    Method for computing nominator for GAP score.   分子
    Args:
        ranked_candidates: List of ranked candidates.
    ...
    for rank, word in enumerate(ranked_candidates):
        ...
        if weight:
            cumsum += weight
            nominator += cumsum / (rank+1)      # 除以排名，分子
    return nominator
...
def compute_gap(
    ...
    cumsum = np.cumsum(list(gold_mapping.values())) # weights的累计和

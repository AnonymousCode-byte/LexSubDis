...
class SubstituteGenerator:
    ...
    def get_log_probs(
        ...
        for postprocessor in self.post_processing:
            ...
            log_probs, word2id = postprocessor.transform(
                ...
                target_pos=target_pos_tags,         # 后处理需要用到
            )
    ...
    def get_model_score(self,sentences: List[List[str]] = None,pred_substitutes: List[str]=None,target_ids: List[list[int]]=None):
        # from base_estimator
        token_similarity_score,sentence_similarity_score,attention_score,validation_score=self.prob_estimator.get_model_score(sentences=sentences,
                                                                    pred_substitutes=pred_substitutes,target_ids=target_ids)
        return token_similarity_score,sentence_similarity_score,attention_score,validation_score

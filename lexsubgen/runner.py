...
class Runner:
    ...
    def evaluate(self,config: Dict[str, Any],experiment_name: Optional[str] = None,run_name: Optional[str] = None):
        ...
        # print(f"从配置文件实例化了对象，读取配置没问题，substgen_config 的类型: {type(substgen_config)}")
        # print(f"从配置文件实例化了对象，dataset_config 的类型: {type(dataset_config)}")
        ...
        # 执行评测逻辑
        dataset = lexsub_eval.dataset_reader.read_dataset() # 读取数据

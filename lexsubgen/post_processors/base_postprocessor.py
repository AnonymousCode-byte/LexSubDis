...
# transform 方法：这是一个抽象方法，用于转换概率估计器的预测结果和词汇表。
class PostProcessor:
    def __init__(self):
        """
    ...
    # LowercasePostProcessor 类可以将词汇表中大写和小写形式的单词合并，并根据指定的策略（sum 或 max）对概率进行聚合。
    # 在原始的 BERT-base 和 BERT-large 模型中，词汇表是区分大小写的。也就是说，
    # 像 “Apple” 和 “apple” 会被视为不同的词，在词汇表中拥有不同的索引。 China和china
